...
The build requires an MPI implementation, GNU Gengetopt, the NVIDIA CUDA toolkit, and the header-only
...
By default, the simulator expects eight ranks using GPUs 0 through 7, simulates
24 qubits, and runs 64 benchmark samples. Use
...
```sh
mpirun -n 4 bin/qcs -g 0,1,2,3 -q 30 -s 100
# Equivalent: mpirun -n 4 bin/qcs --gpus 0,1,2,3 --num-qubits 30 --num-samples 100
```

The GPU list must contain 1, 2, 4, or 8 entries and exactly one entry per MPI
rank. Each rank owns one state split on its assigned GPU. The implementation
uses CUDA IPC to map the other ranks' state splits and assumes all ranks run on
one node. Run `bin/qcs -h` to display the command-line help.
